...
class Quaternion():
    ...
    ###################
    @classmethod
    def from_angle(cls, angle: float, vector: Iterable[float], degrees: bool = True) -> Quaternion:
        if degrees:
            angle_in_radians = _radians(angle)
        else:
            angle_in_radians = angle
            
        halfAngle = angle_in_radians * 0.5
        cosAngle = _cos(halfAngle)
        sinAngle = _sin(halfAngle)
        
        return Quaternion(cosAngle, vector[0] * sinAngle, vector[1] * sinAngle, vector[2] * sinAngle)

    def RotateVector( cls, vector: Vector3 ) -> Vector3:
        p = Quaternion(0.0, vector.x, vector.y, vector.z)
        q = cls
        p_prime = q * p * q.inverse()
        return Vector3( p_prime.i, p_prime.j, p_prime.k )
